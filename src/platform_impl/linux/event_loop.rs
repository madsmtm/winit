...
impl<T: 'static> EventLoop<T> {
    ...
    pub fn new_any_thread() -> EventLoop<T> {
        ...
        context
            .with_thread_default(|| {
                EventLoop::new_gtk().expect("Failed to initialize gtk backend!")
            })
            ...
            if let Some(window) = app_.window_by_id(id.0) {
                ...
                match request {
                    WindowRequest::GlobalHotKey(hotkey_id) => {
                        if let Err(e) =
                            event_tx.send(Event::GlobalShortcutEvent(AcceleratorId(hotkey_id)))
                        {
                            log::warn!(
                                "Failed to send global hotkey event to event channel: {}",
                                e
                            );
                        }
                    }
